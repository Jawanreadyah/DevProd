...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  return (
    <section
      className="w-full max-w-xl bg-white border border-gray-100 grid p-1.5 sm:p-3 gap-0.5 sm:gap-1 rounded-[10px] sm:rounded-[15px]"
      style={{
      ...
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              `border border-gray-100 h-full w-full aspect-square rounded-[4px] sm:rounded-[3px]`,
              isHighlighted ? "animate-highlight" : "",
              shouldFlash ? "animate-flash" : "",
              !isHighlighted && !shouldFlash ? "bg-gray-50" : ""
            )}
            style={
              {
