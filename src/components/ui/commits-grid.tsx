...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  return (
    <section
      className="w-full max-w-xl bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-white/5 grid p-1.5 sm:p-3 gap-0.5 sm:gap-1 rounded-[10px] sm:rounded-[15px] transition-colors duration-300"
      style={{
      ...
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              `border border-gray-100 dark:border-white/5 h-full w-full aspect-square rounded-[4px] sm:rounded-[3px]`,
              isHighlighted ? "animate-highlight" : "",
              shouldFlash ? "animate-flash" : "",
              !isHighlighted && !shouldFlash ? "bg-gray-50 dark:bg-[#222]" : ""
            )}
            style={
              {
                animationDelay: getRandomDelay(),
                "--highlight": getRandomColor(),
                "--card": "var(--commits-card-bg, #f9fafb)",
              } as CSSProperties
            }
          />
        );
